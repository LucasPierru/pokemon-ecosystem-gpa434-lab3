...
P_Grass::P_Grass(QPointF const & initialPosition, size_t intelligence,
	qreal initialSpeed, qreal scale, QParameters *parameter, QGraphicsItem * parent)
{
	setParameter(parameter);
	setLife(parameter->getMaxHealth());
	setNature(parameter->getNature());
	mIntelligence = intelligence;
	setSpeed(parameter->getPokemonSpeed());
	setSize(scale);
	setSpeed(parameter->getPokemonSpeed());
	meteoChange(parameter, 3);
	setPos(initialPosition);
	setScale(scale);
}

void P_Grass::advance(int phase)
{
	if (phase == 0) {
		QPointF target;
		qreal decision;
		...
		for (QGraphicsItem * item : scene()->items()) {
			P_Grass* ally{ dynamic_cast<P_Grass*>(item) };
			P_Fire* predator{ dynamic_cast<P_Fire*>(item) };
			P_Water* prey{ dynamic_cast<P_Water*>(item) };
		...
		qreal NatureElement = mNature / 100.0;
		//Convertis la nature du Pokemon en decimal

		if (mNature > 50) {
			NatureElement = 0.3 + ((mNature / 100.0) / 2);
		}
		else {
			NatureElement = 0.3 - (0.5 - (mNature / 100.0) / 2);
		}
		...
		if (mBreed == 1) {
			target = QPointF((right + left) / 2, bottom - mParameter->getAreaTGrass());
		}
		if (mIntelligence == 2 && isInEnemyTerritory()) {
			qreal center = static_cast<qreal>(mParameter->getAreaTFire());
			target.setX(this->x() + (this->x() - left + center));
			target.setY(this->y() + (this->y() - top + center));
		}
		...
		setPos(newPosition);
		//On verifie tout les item de la scene pour voir leurs interactions avec le P_Grass
		for (QGraphicsItem * item : scene()->items()) {
			...
			//Si le Pokemon herbe rencontre un pokemon eau, et qu'aucun des deux est invulnerable
			if (water != nullptr && distance(water->pos()) < (getSize() + water->getSize()) && water->isInvulnerable() == 0 && isInvulnerable() == 0) {
				//Le Pokemon eau perd des vies
				water->reduceLife(mHitByPredator);
				//Le Pokemon herbe perd un peu moins de vies
				reduceLife(mHitByPrey);
				//Le Pokemon eau deviens invulnerable
				water->setInvulnerablity(1);
				//Si le Pokemon eau meurt, le Pokemon herbe gagne un "kill"
				if (water->getLife() <= 0) {
					mKillCount++;
				}
			}
			//Si le Pokemon herbe est invulnerable, il devient transparent et on lance son timer
			if (isInvulnerable()) {
				setOpacity(0.5);
				timerInvulnerable();
			}
			//Si le Pokemon herbe rencontre un berry, il le mange, regagne une vie et gagne une taille selon son intelligence
			if (berry != nullptr && distance(berry->pos()) < getSize() + berry->getRadius()) {
				eatBreed(mIntelligence);
			}
			//Si le Pokemon herbe est dans son territoire et qu'il doit breed, il cree un enfant et reprend sa taille originale
			if (isInTerritory() && mBreed == 1) {
				P_Grass *babyGrass(new P_Grass(pos(), 0, mParameter->getTikSpeed(), 12.0, mParameter));
				scene()->addItem(babyGrass);
				...
				babyGrass->setHealthBar(mHealth, mEmptyHealth);
...
void P_Grass::paint(QPainter * painter, const QStyleOptionGraphicsItem * option, QWidget * widget)
{
	QPixmap vert;
	...
	painter->drawPixmap(rect, vert);
}

bool P_Grass::isInTerritory()
{
	left = scene()->sceneRect().left();
	right = scene()->sceneRect().right();
...
bool P_Grass::isInEnemyTerritory()
{
	left = scene()->sceneRect().left();
	right = scene()->sceneRect().right();
	top = scene()->sceneRect().top();
